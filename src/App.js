...
function App() {
  ...
  const isValid = (txn) => {
    ...
    const descOk =
      typeof txn?.description === "string" && txn.description.trim().length > 0;
    ...
    try {
      if (!isValid(txn)) {
        window.dispatchEvent(
          new Event(editIndex === null ? "study:add_err" : "study:edit_err")
        );
        setShowModal(false);
        setEditIndex(null);
        return;
      ...
      if (editIndex === null) {
        const withId = txn.id
          ? txn
          : { ...txn, id: crypto?.randomUUID?.() ?? Date.now().toString() };
        ...
        const withId = txn.id
          ...
          : {
              ...txn,
              id: priorId || (crypto?.randomUUID?.() ?? Date.now().toString()),
            };
        setTransactions((prev) =>
          prev.map((t, i) => (i === editIndex ? withId : t))
        );

...
function App() {
  ...
  useEffect(() => { initStudy(); installCustomEventBridges(); }, []);
  ...
  const openEdit = (idx) => { setEditIndex(idx); setShowModal(true); };
  ...
  const handleSubmitTxn = (txn) => {
    try {
      if (!isValid(txn)) {
        ...
        setShowModal(false); setEditIndex(null); return;
      }
      if (editIndex===null) {
      ...
      setShowModal(false); setEditIndex(null);
    } catch {
      window.dispatchEvent(new Event(editIndex===null ? "study:add_err" : "study:edit_err"));
      setShowModal(false); setEditIndex(null);
    }
        ...
        <div className="app">
          ...
          <Routes>
            ...
            <Route path="/dashboard" element={
              <Dashboard transactions={transactions} totals={totals} onOpenAddModal={openAdd} />
            } />
